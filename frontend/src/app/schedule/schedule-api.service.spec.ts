...
describe('ScheduleApiService', () => {
  ...
  it('uses expected schedule API endpoints with credentials', async () => {
    ...
    const complete = service.complete('schedule-id', { participantStudentIds: ['student-1'], notes: 'Boa resposta.' });
    request = http.expectOne('/api/schedule/schedule-id/complete');
    expect(request.request.method).toBe('POST');
    expect(request.request.body).toEqual({ participantStudentIds: ['student-1'], notes: 'Boa resposta.' });
    expect(request.request.withCredentials).toBe(true);
    request.flush({});
    await complete;
  });
